...
components = [
    ["diff_pair_test", diff_pair],
    ["nmos_test", nmos],
    ["pmos_test", pmos]
]

for comp in components:
    mynet, comp = get_gds_netlist(comp[0], comp[1], sky130, gds_path)
    ...
    if sub.returncode != 0:
        print(f'LVS failed for {comp[0]} with error:\n {stderr}')
    else:
        print(f'LVS run successful for {comp[0]}')