...
def simulate_component(comp, pdk):
    with Temp.TemporaryDirectory() as tmpdirname:
        ...
        # shutil.copy(str(gds_file), str(Path(__file__).resolve().parent / 'opamp_test.gds'))
        net = comp.info['netlist'].generate_netlist()
        with open(cdl_file, 'w') as f:
            f.write(net)
        # shutil.copy(str(cdl_file), str(Path(__file__).resolve().parent / 'opamp_test.cdl'))
        result = pdk.drc_magic(gds_file, comp.name)
        drc_result = validate_drc_result(result, comp.name)

        result = pdk.lvs_netgen(gds_file, comp.name, netlist=cdl_file, report_handling=(report_path))
...
if args.component == 'opamp':
    comp = sky130_add_opamp_labels(opamp(sky130_mapped_pdk))
