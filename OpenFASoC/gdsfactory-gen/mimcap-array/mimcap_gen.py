...
parser.add_argument('--wtop', required=True,
                    help='Top metal width')
parser.add_argument('--wbot', required=True,
                    help='Bottom metal width')
parser.add_argument('--lyrtop', required=True,
                    help='Top metal layer number')
...
while (i <= length): 
    ...
    pt2 = (i-Cstructure_in.xsize/2+top_width/2, length + Cstructure_in.xsize/2)
...
while (j <= length): 
    ...
    pt2 = (length + Cstructure_in.xsize/2, j+Cstructure_in.xsize/2-top_width/2)
...
while (i <= length): 
    ...
    pt2 = (i+Cstructure_in.xsize/2-bot_width/2, length + Cstructure_in.xsize/2)
...
while (j <= length): 
    ...
    pt2 = (length + Cstructure_in.ysize/2, j-Cstructure_in.xsize/2+bot_width/2)
...
top_rail_pts = [(left_edge, top_edge - top_width), (right_edge, top_edge - top_width), (right_edge, top_edge + allowed_space - top_width), (left_edge, top_edge + allowed_space - top_width)]
bot_rail_pts = [(left_edge, bot_edge + bot_width), (right_edge, bot_edge + bot_width), (right_edge, bot_edge - allowed_space + bot_width), (left_edge, bot_edge - allowed_space + bot_width)]
...
connector_pts = [(left_edge, top_edge - top_width + 1/2 * connector_width), (left_edge - 6, top_edge - top_width + 1/2 * connector_width), (left_edge - 6, bot_edge - 5), (left_edge, bot_edge - 5)]
...
# OUTPUT
Cstructure.write_gds('merged_' + str(top_layer-1) + '_' + str(top_layer) + '_cap_array.GDS')