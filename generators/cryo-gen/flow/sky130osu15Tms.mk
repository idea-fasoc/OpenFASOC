...
PUBLIC=sky130hd sky130hs sky130hvl sky130osu12Ths sky130osu18Ths sky130osu15Tms
...
$(RESULTS_DIR)/2_2_floorplan_io.def: $(RESULTS_DIR)/2_1_floorplan.def
	($(TIME_CMD) $(OPENROAD_CMD) $(SCRIPTS_DIR)/io_placement.tcl -metrics $(LOG_DIR)/2_2_floorplan_io.json) 2>&1 | tee $(LOG_DIR)/2_2_floorplan_io.log
...
$(RESULTS_DIR)/2_floorplan_ro.def: $(RESULTS_DIR)/2_floorplan.def
	python3 util/place_ro.py --inputDef $(RESULTS_DIR)/2_floorplan.def --outputDef $(RESULTS_DIR)/2_floorplan_ro.def --coreDim 58,30 --arrayDim 36,4 --cellDim 0.11,5.55 --targetInst cryo_ro_1 --coreDieOffset 3.63,5.55

# STEP 1: Global placement + IO placement (not random)
#-------------------------------------------------------------------------------
$(RESULTS_DIR)/3_1_place_gp.def: $(RESULTS_DIR)/2_floorplan_ro.def $(RESULTS_DIR)/2_floorplan.sdc
...
ifeq ($(USE_WXL),)
	...
	echo "bottomRoutingLayer:met1" >> $@
	echo "topRoutingLayer:met5" >> $@ 
	echo "cpxthreads:1" >> $@
	echo "verbose:1" >> $@
	echo "gap:0" >> $@
	echo "timeout:2400" >> $@
...
netgen_lvs: $(RESULTS_DIR)/6_final.gds
	...
	cdl_spice_file = $(subst sky130, sky130_fd_sc_, $(PLATFORM)).spice
        
	python util/cdl_parser.py -i $(RESULTS_DIR)/6_final.cdl -s $(PLATFORM_DIR)/cdl/$(cdl_spice_file) -l $(OBJECTS_DIR)/merged_spacing.lef -o $(DESIGN_NAME).spice
