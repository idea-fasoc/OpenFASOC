...
def generate_runs(genDir, designName, headerList, invList, tempList, jsonConfig, platform, modeling=False):
  ...
  platformSpice = glob.glob(genDir + "../../common/platforms/%s/cdl/*.spice" % (platform))
  ...
  with open(simDir + "templates/tempsenseInst_%s.sp" % (simTool), "r") as rf:
    ...
    simTestbench = re.sub("@model_corner", platformConfig["model_corner"], simTestbench)
    simTestbench = re.sub("@voltage", str(platformConfig["nominal_voltage"]), simTestbench)
  ...
  for design in designList:
    ...
    if modeling:
      ...
      simTestbench = re.sub("@netlist", os.path.abspath(runDir + 'TEMP_sensor_inv%d_header%d.spice' % (inv, header)), simTestbench)
      ...
      simTestbench = re.sub("@netlist", os.path.abspath(runDir + designName + '.spice'), simTestbench)
...
def update_netlist(srcNetlist, dstNetlist, simMode):
  with open(srcNetlist, "r") as rf:
    ...
    if simMode == "partial":
      ...
      print(simMode + " is not a valid mode for simulation, only partial and full modes are supported")
      sys.exit(1)
  with open(dstNetlist, "w") as wf:
    wf.write(netlist)
...
def run_simulations(runDir, designName, temp_list, simTool, simMode):
  if simTool == "finesim":
    ...
    with open(runDir + "cal_result", "w") as wf:
      for temp in temp_list:
        wf.write("python result.py --tool finesim --inputFile %s_sim_%d.log\n" % (designName, temp))
    ...
    for temp in temp_list:
      if not os.path.isfile(runDir + "%s_sim_%d.log" % (designName, temp)):
        ...
        sys.exit(1)
      p = sp.Popen(["python", "result.py", "--tool", "finesim", "--inputFile", "%s_sim_%d.log" % (designName, temp)], cwd=runDir)
      p.wait()
