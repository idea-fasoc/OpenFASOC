...
def HEADER_process(netlist):
	...
	correct_HEADER_pinout = correct_HEADER_pinout.replace(" sky130_fd_sc_hd__tap_1_1/VPB","")
	...
	if "sky130_fd_sc_hd__tap_1_0/VPB" in HEADER_pinout_arr:
		index_0 = HEADER_pinout_arr.index("sky130_fd_sc_hd__tap_1_0/VPB")# first removed pin
	if "sky130_fd_sc_hd__tap_1_1/VPB" in HEADER_pinout_arr:
		index_1 = HEADER_pinout_arr.index("sky130_fd_sc_hd__tap_1_1/VPB")# first removed pin
	...
	for HEADER_instance in HEADER_instances_m:
		...
		if index_0+1:# check if the index was defined to something (cannot be -1 and -1+1=false)
...
parser = argparse.ArgumentParser(description="remove the proxy pins from extracted HEADER cell definition")
parser.add_argument("--lvsmag", "-l", required=True, help="extracted spice file")
parser.add_argument("--toplevel", "-t", required=False, help="name of toplevel module to look for")# there may not be a toplevel
