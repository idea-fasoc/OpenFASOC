...
def __get_output_filepath(filename, out_dir):
	return path.join(out_dir, filename.replace(".template", ""))

def __generate_file(input_path, output_path, parameters):
	template = Template(filename=input_path)

	out_file = open(output_path, "w")
	out_file.write(template.render(**parameters))

def __generate_subdirectory(src_dir, out_dir, parameters):
...
def generate_verilog(
	parameters,
	src_dir = "src",
	out_dir = path.join("flow", "design", "src")
):
