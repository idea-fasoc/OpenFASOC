...
for stage_num in range(0,6):
    ...
    for cell_num in range(len(six_stages[stage_num]["DCDC_CAP_UNIT_L"])):
        ...
        cell_actual_coord = (cell_array_coord[0] * conv21_cell_w + stage_offset_x + conv21_cell_w/4 - cap_cell_w/2, -(cell_array_coord[1] + 2) * cap_cell_h)
    ...
    for cell_num in range(len(six_stages[stage_num]["DCDC_CAP_UNIT_R"])):
        # place cells (x, y) in a tuple
        cell_array_coord = (cell_num % conv21_array_dim_x, cell_num // conv21_array_dim_x)
        cell_actual_coord = (cell_array_coord[0] * conv21_cell_w + stage_offset_x + conv21_cell_w*3/4 - cap_cell_w/2, -(cell_array_coord[1] + 2) * cap_cell_h)
    ...
    for cell_num in range(len(six_stages[stage_num]["DCDC_MUX"])):
        ...
        cell_actual_coord = (cell_array_coord[0] * mux_cell_w + stage_offset_x, cell_array_coord[1] * mux_cell_h + conv21_height_ref * conv21_cell_h + 5)
        ...
        # store placement into dictionary
        six_stages[stage_num]["DCDC_MUX"][cell_num].extend([cell_actual_coord[0], cell_actual_coord[1]])

    # update offset for next stage
    stage_offset_x += (conv21_array_dim_x ) * conv21_cell_w + 5
