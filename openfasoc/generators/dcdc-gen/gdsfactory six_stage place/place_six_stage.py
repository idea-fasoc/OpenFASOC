...
def label(loc, layer, text):
    ...
    six_stage_gf_com.add_polygon([(loc[0]-0.01, loc[1]-0.01), (loc[0]+0.01, loc[1]-0.01), (loc[0]+0.01, loc[1]+0.01), (loc[0]-0.01, loc[1]+0.01)], layer=(layer, 16))
...
for stage_num in range(0,6):
    ...
    conv21_array_dim_x = math.ceil(len(six_stages[stage_num]["DCDC_CONV2TO1"])/conv21_height_ref)
    
    for cell_num in range(len(six_stages[stage_num]["DCDC_CAP_UNIT_L"])):
        # place cells (x, y) in a tuple
        cell_array_coord = (cell_num % conv21_array_dim_x, cell_num // conv21_array_dim_x)
        cell_actual_coord = (cell_array_coord[0] * conv21_cell_w + stage_offset_x + conv21_cell_w/4 - cap_cell_w/2, -(cell_array_coord[1] + 1) * cap_cell_h)
        ...
        six_stages[stage_num]["DCDC_CAP_UNIT_L"][cell_num].extend([cell_actual_coord[0], cell_actual_coord[1]])

    for cell_num in range(len(six_stages[stage_num]["DCDC_CAP_UNIT_R"])):
        # place cells (x, y) in a tuple
        cell_array_coord = (cell_num % conv21_array_dim_x, cell_num // conv21_array_dim_x)
        cell_actual_coord = (cell_array_coord[0] * conv21_cell_w + stage_offset_x + conv21_cell_w*3/4 - cap_cell_w/2, -(cell_array_coord[1] + 1) * cap_cell_h)
        ...
        # store placement into dictionary
        six_stages[stage_num]["DCDC_CAP_UNIT_R"][cell_num].extend([cell_actual_coord[0], cell_actual_coord[1]])
    ...
    for cell_num in range(grid_count):
        # place cells (x, y) in a tuple
        cell_array_coord = (cell_num % conv21_array_dim_x, cell_num // conv21_array_dim_x)
        cell_actual_coord = (cell_array_coord[0] * conv21_cell_w + stage_offset_x, cell_array_coord[1] * conv21_cell_h)
        ...
        if (cell_array_coord[1] + 1 == conv21_height_ref):
            ...
            adapter_actual_coord = (adapter_array_coord[0] * conv21_cell_w + stage_offset_x, adapter_array_coord[1] * conv21_cell_h)
        ...
        if (cell_num < len(six_stages[stage_num]["DCDC_CONV2TO1"])):
            ...
            # store placement into dictionary
            six_stages[stage_num]["DCDC_CONV2TO1"][cell_num].extend([cell_actual_coord[0], cell_actual_coord[1]])
        else:
            # place cell using gdsfactory
    ...
    for cell_num in range(len(six_stages[stage_num]["DCDC_MUX"])):
        ...
        cell_actual_coord_mux = (cell_array_coord[0] * mux_cell_w + stage_offset_x,cell_array_coord[1] * mux_cell_h + conv21_height_ref * conv21_cell_h + conv21_cell_adapter_h)
        ...
        # store placement into dictionary
        six_stages[stage_num]["DCDC_MUX"][cell_num].extend([cell_actual_coord_mux[0], cell_actual_coord_mux[1]])
    ...
    label((stage_offset_x + sel_h_offset[0], conv21_height_ref * conv21_cell_h + conv21_cell_adapter_h + sel_h_offset[1]), 70, f"SEL_H[{stage_num}]")
    label((stage_offset_x + sel_l_offset[0], conv21_height_ref * conv21_cell_h + conv21_cell_adapter_h + sel_l_offset[1]), 70, f"SEL_L[{stage_num}]")
...
label((stage_offset_x - inter_stage_w - 0.5, conv21_height_ref * inter_stage_h + 10), 70, f"OUT")
        ...
        for stage_num in range(0, 6):
            for auxcell in six_stages[stage_num]:
                for inst in six_stages[stage_num][auxcell]:
                    ...
                    complete_line_split.extend(["R0", "{:.3f}".format(value[3] + offset_x), "{:.3f}".format(value[4] + cap_section_offset + offset_y), "\n"])
