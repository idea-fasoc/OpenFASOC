...
class MappedPDK(gf.pdk.Pdk):
	...
	valid_glayers: ClassVar[list[str]] = [
		"dnwell","pwell","nwell","p+s/d", "n+s/d", "active", "poly",
		"mcon", "met1", "via1", "met2", "via2", "met3", "via3", "met4"
		]
	...
	def glayers_check_keys(cls, glayers_obj: dict[StrictStr,StrictStr]):
		...
		for glayer, mapped_layer in glayers_obj.items():
			...
			if glayer not in cls.valid_glayers:
				raise ValueError("glayers keys must be one of generic layers listed in class variable valid_glayers")
		return glayers_obj
	...
	def drc(self, layout: gf.typings.Component | gf.typings.PathType, output_dir_or_file: Optional[gf.typings.PathType] = None):
		...
		report_path = Path(output_dir_or_file).resolve() if output_dir_or_file else Path.cwd().resolve()
		if report_path.is_dir():
			report_path = Path(report_path / str(self.name + layout_path.name.replace(layout_path.suffix,"") + "_drcreport.lyrdb"))
		elif not report_path.is_file():
			raise ValueError("report_path must be file or dir")
		# run klayout drc
		drc_args = ["klayout", "-b",
			"-r",str(self.klayout_lydrc_file_path),
	...
	def has_required_glayers(self, layers_required: list[str]):
		...
		for layer in layers_required:
			if layer not in self.glayers:
				raise ValueError(f"{layer!r} not in Pdk.glayers {list(self.glayers.keys())}")
