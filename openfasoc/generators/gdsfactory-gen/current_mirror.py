...
def nmos() -> Component:
    ...
    met1_label_s = c.add_label(
        "S", position=(0.255, 0.36), layer=met1_label, magnification=0.2
    )
    met1_label_d = c.add_label(
        "D", position=(0.81, 0.36), layer=met1_label, magnification=0.2
    )
    # c.add_label()
...
def cmirror_top(mult=3) -> Component:
    ...
    for i in range(mult):
        for j in range(2):
            ...
            met1_row_conn_rect = gf.components.rectangle(
                size=(met1_width, met1_height), layer=met1_drawing
            )
    ...
    met2_center = gf.components.rectangle(
        size=(met2_center_width, met2_center_height), layer=met2_drawing
    )
    ...
    met2_label_I_in = Top_cell.add_label(
        "VSS", position=(-0.5, 0.9 - 0.03), layer=met2_label, magnification=0.2
    )
    ...
    met2_label_I_in = Top_cell.add_label(
        "I_in", position=(-0.5, 2.5), layer=met2_label, magnification=0.2
    )
    ...
    met2_label_I_out = Top_cell.add_label(
        "I_out", position=(-0.5, 2.5 + 0.5), layer=met2_label, magnification=0.2
    )
    ...
    via_rect_ref.movex(trunk_1_x_shift + 0.03).movey(
        met1_right_height_2 - 0.73 - 0.17 - 0.03
    )

    via_rect_ref = Top_cell << via_rect
    via_rect_ref.movex(trunk_2_x_shift + 0.03).movey(-1.23 + 0.03)
    via_rect_ref = Top_cell << via_rect
    via_rect_ref.movex(trunk_2_x_shift + 0.03).movey(
        met1_right_height_1 - 1.23 - 0.17 - 0.03
    )
        ...
        for j in range(2):
            ...
            if i % 2 != 0:
                if j % 2 != 0:
                    ...
                    met1_tr_conn_rect_ref.movex(cell_width * i + 0.16 + 0.55).movey(
                        -1.23
                    )

                    via_rect_ref = Top_cell << via_rect
                    via_rect_ref.movex(cell_width * i + 0.16 + 0.55 + 0.03).movey(
                        -1.23 + 0.03
                    )
                else:
                    ...
                    via_rect_ref = Top_cell << via_rect
                    via_rect_ref.movex(cell_width * i + 0.16 + 0.55 + 0.03).movey(
                        1.455 + met1_tr_conn_height_1 - 0.17 - 0.03
                    )
            else:
                if j % 2 != 0:
                    met1_tr_conn_rect_ref = Top_cell << met1_tr_conn_rect_1
                    ...
                    via_rect_ref = Top_cell << via_rect
                    via_rect_ref.movex(cell_width * i + 0.16 + 0.55 + 0.03).movey(
                        1.455 + met1_tr_conn_height_2 - 0.17 - 0.03
                    )
    return Top_cell
