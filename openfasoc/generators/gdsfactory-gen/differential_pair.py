...
@gf.cell
def nmos(cell_height) -> Component:
    c = Component()
    ...
    ##poly
    poly_width = 0.25
    poly_height = cell_height + 0.01  ##0.68
    ...
    for i in range(int(cell_height / 0.67)):
        ...
        mcon_rect_ref2.movey(0.67 * i + 0.25).movex(0.74)
    ...
    for i in range(int(cell_height / 0.67)):
        ...
        licon1_rect_ref2.movey(0.67 * i + 0.25).movex(0.74)
    ...
    met1_label_s = c.add_label(
        "S", position=(0.255, (cell_height / 2)), layer=met1_label, magnification=0.2
    )
    met1_label_d = c.add_label(
        "D", position=(0.81, (cell_height / 2)), layer=met1_label, magnification=0.2
    )
    # c.add_label()
...
@gf.cell
def diff_pair_top(mult=3, cell_height=0.67) -> Component:

    Top_cell = gf.Component("top")

    mult = mult * 2
    mos_comp = nmos(cell_height)
    # cell_height = 0.67
    cell_width = 1.1
    space_bet_rows = 0.66
    # mult = 8
    ...
    ##pwell
    pwell_width = (cell_width * ((mult + 1) / 2)) + 0.11
    pwell_height = (cell_height * 1) + 0.11
    ...
    dnwell_height = (cell_height * 1) + 0.91
    ...
    for i in range(mult):
        ...
        if i == 0:
            ...
            ref.movex(cell_width * i - (cell_width * i / 2)).movey(
                cell_height * (j) + space_bet_rows * (j)
            )
    ...
    for i in range(mult):
        ...
        if i % 2 == 0:
            ...
            poly_row_conn_rect = gf.components.rectangle(
                size=(poly_width, poly_height), layer=poly_drawing
            )
            poly_row_conn_rect_ref = Top_cell << poly_row_conn_rect

            poly_row_conn_rect_ref.movex(
                cell_width * i - (cell_width * i / 2) + 0.425
            ).movey(-0.1)
    ...
    met2_pin_rect = gf.components.rectangle(
        size=(met2_pin_width, met2_pin_height), layer=met2_pin
    )

    poly_pin_width = poly_pin_height = 0.25
    poly_pin_rect = gf.components.rectangle(
        size=(poly_pin_width, poly_pin_height), layer=poly_pin
    )
    ...
    poly_tb_ref = gf.components.rectangle(
        size=(poly_tb_width, poly_tb_height), layer=poly_drawing
    )
    ## Top poly trunk
    ...
    poly_label_G_M1 = Top_cell.add_label(
        "G_M1",
        position=(-0.5 + 0.25, cell_height + 0.1 + 0.25),
        layer=poly_label,
        magnification=0.2,
    )
    ...
    poly_label_G_M2 = Top_cell.add_label(
        "G_M2",
        position=(-0.5 + 0.25, -0.23 - 0.1 + 0.25),
        layer=poly_label,
        magnification=0.2,
    )
    ...
    met2_tb = gf.components.rectangle(
        size=(met2_tb_width, met2_tb_height), layer=met2_drawing
    )
    ...
    met2_label_I_in = Top_cell.add_label(
        "VSS",
        position=(-0.5 + 0.25, cell_height + 0.5 + 0.2),
        layer=met2_label,
        magnification=0.2,
    )
    ...
    met2_label_I_out = Top_cell.add_label(
        "D_M1",
        position=(-0.5 + 0.25, cell_height + 1.0 + 0.25),
        layer=met2_label,
        magnification=0.2,
    )
    ...
    met2_label_I_in = Top_cell.add_label(
        "D_M1",
        position=(-0.5 + 0.25, -0.5 - 0.23 - 0.5 + 0.25),
        layer=met2_label,
        magnification=0.2,
    )
    ...
    met1_right_trunk_1 = gf.components.rectangle(
        size=(met1_right_width, met1_right_height_1), layer=met1_drawing
    )
    met1_right_trunk_2 = gf.components.rectangle(
        size=(met1_right_width, met1_right_height_2), layer=met1_drawing
    )
    ...
    via_rect_ref = Top_cell << via_rect
    via_rect_ref.movex(trunk_1_x_shift + 0.03).movey(
        met1_right_height_1 - (0.5 + 0.23) - 0.17 - 0.03
    )
    ...
    for i in range(mult):
        ...
        via_rect = gf.components.rectangle(
            size=(via_height, via_width), layer=via_drawing
        )

        met1_tr_conn_rect_1 = gf.components.rectangle(
            size=(met1_tr_conn_width, met1_tr_conn_height_1), layer=met1_drawing
        )

        met1_tr_conn_rect_2 = gf.components.rectangle(
            size=(met1_tr_conn_width, met1_tr_conn_height_2), layer=met1_drawing
        )
        ...
        if i == 0:
            ...
            via_rect_ref = Top_cell << via_rect
            via_rect_ref.movex(0.16 + 0.03).movey(
                0.125 + met1_tr_conn_height_2 - 0.17 - 0.03
            )
        ...
        if i % 2 != 0:
            ...
            if connect == "up":
                ...
                print(i)
                met1_tr_conn_rect_ref = Top_cell << met1_tr_conn_rect_2
                met1_tr_conn_rect_ref.movex(
                    cell_width * i - (cell_width * i / 2) + 0.16 + 0.55
                ).movey(-1.23)

                via_rect_ref = Top_cell << via_rect
                via_rect_ref.movex(
                    cell_width * i - (cell_width * i / 2) + 0.16 + 0.55 + 0.03
                ).movey(-1.23 + 0.03)
                connect = "down"
            else:
                print("\nElse ---- > Mult position")
                ...
                via_rect_ref.movex(
                    cell_width * i - (cell_width * i / 2) + 0.16 + 0.55 + 0.03
                ).movey(0.125 + met1_tr_conn_height_2 - 0.17 - 0.03)
                connect = "up"
        ...
        if i % 2 == 0:
            if i % 4 != 0:
                ...
                via_rect_ref.movex(
                    cell_width * i - (cell_width * i / 2) + 0.16 + 0.55 + 0.03
                ).movey(0.125 + met1_tr_conn_height_1 - 0.17 - 0.03)

    return Top_cell


## Top cell creation
Top_cell = diff_pair_top(5, 1.34)
Top_cell.show()
