...
for i in range(len(cells)):
    ...
    created_cells.append(createPCellInstance(
        layout=__layout__,
...
for i in range(len(cells)):
    ...
    if (curCell['pcell'] == "diff_octagon_inductor") or (curCell['pcell'] == "diff_square_inductor"):
        ...
        __component__.add_port(
            ...
            width=width, orientation=270, layer=LAYER.met5drawing
        )
        __component__.add_port(
        ...
        consoleOut(f"Defined Ports {curCell['ports'][0]} and {curCell['ports'][1]} for {list(cells.keys())[i]}")
        # Create Pads
        pads_array = [ ]
        if "pads" in curCell:
            ...
            if curCell['pads']['type'] == 'GSGSG':
                for j in range(5):
                    ...
                    pad.add_port(
                        name=f"io{j}", center=[20, 40], width=40, orientation=90, layer=LAYER.met5drawing
                    )
                    pad = PADS << pad 
                    pad.move((((curCell['pads']['pitch']*j) - ((2*curCell['pads']['pitch'])+20)), (curCell['position'][1]-curCell['pads']['padding'])))
            ...
            routeInput = gf.routing.get_route_electrical(
                __component__.ports[f"{list(cells.keys())[i]}_{curCell['ports'][0]}"], pads_array[1].ports["io1"], layer=LAYER.met5drawing, width = width
            )
            __top_level__.add(routeInput.references)
            routeOutput = gf.routing.get_route_electrical(
                __component__.ports[f"{list(cells.keys())[i]}_{curCell['ports'][1]}"], pads_array[3].ports["io2"], layer=LAYER.met5drawing, width = width
            )
