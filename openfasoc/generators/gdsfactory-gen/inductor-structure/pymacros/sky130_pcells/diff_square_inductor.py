...
class diff_squar_ind_Generator(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
        ...
        self.param(
            "distance_input",
            self.TypeDouble,
            "Distance of input conductors",
            default=10,
        )
        self.param(
            "spacing_input",
            self.TypeDouble,
            "Spacing of input inductors conductors",
            default=8,
        )

        self.param("Louter", self.TypeDouble, "outer dimension", default=60)

        shielding_option = self.param(
            "shielding", self.TypeString, "Shielding Type", default="No shielding"
        )
        ...
        self.param(
            "W_shielding",
            self.TypeDouble,
            "Width of the conductors for shielding",
            default=2,
        )
        self.param(
            "S_shielding",
            self.TypeDouble,
            "Spacing between conductors for shielding",
            default=4,
        )
        ...
        self.param(
            "diffusion_shielding", self.TypeBoolean, "Diffusion shielding", default=0
        )
    ...
    def rectangular_shielding(self, W, S, Lver, Lhor, diffusion, input_distance):
        ...
        for i in range(N_Conductors):
            ...
            self.cell.shapes(met1).insert(
                pya.Box(xcor, ycor, xcor + W, ycor + Lver)
            )  # draw the metal conductors
            if Shielding_with_diffusion == 1:  # inserting diffusion layer if it's true
                ...
                for j in range(N_Licon_ver):  # drawing the licon in yaxis
                    ycor_Licon = (
                        ...
                        + j * (Licon_Width_Length + Licon_Spacing)
                    )
                ...
                for j in range(N_Mcon_ver):  # drawing the mcon in yaxis
                    ycor_Mcon = (
                        ...
                        + j * (Mcon_Width_Length + Mcon_Spacing)
                    )
    ...
    def triangular_shielding(self, W, S, Lhor, diffusion, input_distance):
        ...
        xcor = xcor + (W + S)
        self.cell.shapes(met1).insert(
            pya.Box(xcor, ycor, -xcor, ycor + Lver / 2 - S / 2)
        )
        self.cell.shapes(met1).insert(
            pya.Box(xcor, ycor + Lver, -xcor, ycor + Lver / 2 + S / 2)
        )
        if Shielding_with_diffusion == 1:
            self.cell.shapes(nwell).insert(
                pya.Box(xcor, ycor, -xcor, ycor + Lver / 2 - S / 2)
            ...
            N_Licon_ver = int(
                ((Lver / 2 - S / 2) - 2 * Li_Encloses_Licon_Two_Sides + Licon_Spacing)
                / (Licon_Width_Length + Licon_Spacing)
            )
            print(N_Licon_ver)
            Remaining_Licon_ver = (
            ...
            N_Mcon_ver = int(
                ((Lver / 2 - S / 2) - 2 * Met1_Encloses_Mcon_Two_Sides + Mcon_Spacing)
                / (Mcon_Width_Length + Mcon_Spacing)
            )
            Remaining_Mcon_ver = (
                (Lver / 2 - S / 2)
        ...
        if (-y_check2 * 2) >= (2 * S + 0.14):
            self.cell.shapes(met1).insert(
                pya.Box(xcor2, y_check + S, x_check, y_check - y_check2 * 2 - S)
            )
            self.cell.shapes(met1).insert(
                pya.Box(xcor2 + Lhor, y_check + S, -x_check, y_check - y_check2 * 2 - S)
            )
            if Shielding_with_diffusion == 1:
                self.cell.shapes(nwell).insert(
                    pya.Box(xcor2, y_check + S, x_check, y_check - y_check2 * 2 - S)
                ...
                self.cell.shapes(nwell).insert(
                    pya.Box(
                        xcor2 + Lhor, y_check + S, -x_check, y_check - y_check2 * 2 - S
                    )
                )
                self.cell.shapes(li1).insert(
                    pya.Box(xcor2, y_check + S, x_check, y_check - y_check2 * 2 - S)
    ...
    def draw_vias(self, x_start, y_start, x_end, y_end, via_number):
        ...
        if via_number == 1:
            ...
            num_via_1, via_free_spc_1 = self.number_spc_contacts(
                width, met_via_enc_1, via_spc, via_size
            )
            num_via_2, via_free_spc_2 = self.number_spc_contacts(
                length, met_via_enc_2, via_spc, via_size
            )
            via_arr = pya.CellInstArray(
                via_cell.cell_index(),
                pya.Trans(
                    pya.Point(via_free_spc_2 / 2, -width / 2 + via_free_spc_1 / 2)
                ),
                pya.Vector(via_spc + via_size, 0),
                pya.Vector(0, via_spc + via_size),
                num_via_2,
                num_via_1,
            )
        ...
        if via_number == 2:
            ...
            num_via2_1, via2_free_spc_1 = self.number_spc_contacts(
                width, met_via2_enc_1, via2_spc, via2_size
            )
            num_via2_2, via2_free_spc_2 = self.number_spc_contacts(
                length, met_via2_enc_2, via2_spc, via2_size
            )
            via2_arr = pya.CellInstArray(
                via2_cell.cell_index(),
                pya.Trans(
                    pya.Point(via2_free_spc_2 / 2, -width / 2 + via2_free_spc_1 / 2)
                ),
                pya.Vector(via2_spc + via2_size, 0),
                pya.Vector(0, via2_spc + via2_size),
                num_via2_2,
                num_via2_1,
            )
        ...
        if via_number == 3:
            ...
            num_via3_1, via3_free_spc_1 = self.number_spc_contacts(
                width, met_via3_enc_1, via3_spc, via3_size
            )
            num_via3_2, via3_free_spc_2 = self.number_spc_contacts(
                length, met_via3_enc_2, via3_spc, via3_size
            )
            via3_arr = pya.CellInstArray(
                via3_cell.cell_index(),
                pya.Trans(
                    pya.Point(
                        x_start + via3_free_spc_2 / 2, y_start + via3_free_spc_1 / 2
                    )
                ),
        ...
        if via_number == 4:
            ...
            num_via4_1, via4_free_spc_1 = self.number_spc_contacts(
                width, met_via4_enc, via4_spc, via4_size
            )
            num_via4_2, via4_free_spc_2 = self.number_spc_contacts(
                length, met_via4_enc, via4_spc, via4_size
            )
            via4_arr = pya.CellInstArray(
                via4_cell.cell_index(),
                pya.Trans(
                    pya.Point(
                        x_start + via4_free_spc_2 / 2, y_start + via4_free_spc_1 / 2
                    )
                ),
    ...
    def produce_impl(self):
        PERCISION = 1000
        if self.shielding == 1:
            self.rectangular_shielding(
                self.W_shielding,
                self.S_shielding,
                self.Louter,
                self.Louter,
                self.diffusion_shielding,
                self.distance_input,
            )

        if self.shielding == 2:
            self.triangular_shielding(
                self.W_shielding,
                self.S_shielding,
                self.Louter,
                self.diffusion_shielding,
        ...
        via_used_between_top_middle_metals = (
            via4  # the via used between the self.cell and middle metal layers
        )
        middle_metal = (
            met4  # the metal layer used by the inductor at the diagonal conductors
        )
        via_used_between_middle_bottom_metals = (
            via3  # the via used between the middle and the bottom metal layers
        )
        bottom_metal = (
            met3  # the metal layer used by the inductor at the output inductors
        )
        ...
        Dist_input = (
            self.distance_input * PERCISION
        )  # the length of the conductor at the two input pairs
        Spacing_input = (
            self.spacing_input * PERCISION
        )  # the spacing between the conductors of the two input pairs
        ...
        for i in range(N):
            ...
            for j in range(4):  # 4 equals number of points in each half turn
                if (
                    j == 0
                ):  # for the first point of the turn xcor is constant and ycor increases by S+W
                    xcor = Spacing_input / 2
                    ycor = Dist_input + (2 * i + 1) * (W / 2) + i * S
                elif j % 2 == 1:  # odd values of points will change xcor
                    if (
                        xpos == 1
                    ):  # imagine one turn if we want to draw the first horizontal side we increase x-axis value by the conductor length
                        xcor = xcor + (
                            LouterCor / 2
                            - (2 * i) * S / 2
                            - (2 * i + 1) * W / 2
                    ...
                    ycor = ycor + (
                        LouterCor - (2 * i + 1) * W - 2 * i * S
                    )  # to draw the first vertical in each turn side we increase ycor
            ...
            if i == N - 1:
                if (
                    N % 2 == 1
                ):  # if the number of turns odd that means the common conductor is at the self.cell and draw the output
                    ycor = ycor + W / 2
                ...
                self.cell.shapes(met5).insert(
                    pya.Box(Spacing_input / 2, ycor, -Spacing_input / 2, ycor - W)
                )
            ...
            # if the number of turns greater than 1 we want to draw the cross shape
                ...
                self.draw_vias(
                    Spacing_input / 2, ycor, Spacing_input / 2 + W, ycor - W, 4
                )
                ycor = ycor - W / 2
                ycor = self.roundto5(ycor)
                ...
                self.cell.shapes(met4).insert(
                    pya.Path(
                        [
                            pya.Point(Spacing_input / 2 + W, ycor),
                            pya.Point(Spacing_input / 2, ycor),
                            pya.Point(-Spacing_input / 2, ycor - W - S),
                            pya.Point(-Spacing_input / 2 - W, ycor - W - S),
                        ],
                        W,
                    )
                )
                ...
                self.cell.shapes(met5).insert(
                    pya.Path(
                        [
                            pya.Point(-Spacing_input / 2 - W, ycor + S + W),
                            pya.Point(-Spacing_input / 2, ycor + W + S),
                            pya.Point(Spacing_input / 2, ycor),
                            pya.Point(Spacing_input / 2 + W, ycor),
                        ],
        ...
        self.cell.shapes(met5).insert(
            pya.Box(Spacing_input / 2, 0, Spacing_input / 2 + W, Dist_input)
        )
        self.cell.shapes(met5).insert(
            pya.Box(-Spacing_input / 2, 0, -Spacing_input / 2 - W, Dist_input)
        )
