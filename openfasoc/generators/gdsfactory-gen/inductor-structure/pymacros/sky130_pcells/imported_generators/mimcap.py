...
class mimcap:
    ...
    def __init__(self, layout, w, l, pin0="p0", pin1="n0", connection_labels=1):
        ...
        self.l_met4_label = self.layout.layer(met4_label_lay_num, met4_label_lay_dt)
    ...
    def draw_vias(self, box, via3_cell):
        ...
        via3_arr = pya.CellInstArray(
            ...
            pya.Trans(
                pya.Point(
                    box.p1.x + via3_free_spc_1 / 2, box.p1.y + via3_free_spc_2 / 2
                )
            ),
    ...
    def draw_cap(self):
        ...
        self.met4_center_box = mimcap_box.enlarge(
        ...
        met3_box = pya.Box(
            ...
            mimcap.mimcap_drawing_offest * self.percision
            + self.w
            + mimcap.metal3_margin_right * self.percision,
            2 * mimcap.mimcap_drawing_offest * self.percision + self.l,
        )

        self.cell.shapes(self.l_met3).insert(met3_box)
        pin0_text = pya.Text(
            self.pin0, self.met4_center_box.center().x, self.met4_center_box.center().y
        )
        prbndry_box = pya.Box(
            0,
            0,
        ...
        self.met4_side_box = pya.Box(
            prbndry_box.p2.x - mimcap.met4_side_overlap * self.percision,
            prbndry_box.p1.y + 0.06 * self.percision,
            prbndry_box.p2.x
            - mimcap.met4_side_overlap * self.percision
            + mimcap.met4_side_width * self.percision,
            prbndry_box.p2.y - 0.06 * self.percision,
        )
        pin1_text = pya.Text(
            self.pin1, self.met4_side_box.center().x, self.met4_side_box.center().y
        )
