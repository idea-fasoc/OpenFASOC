...
class nmos18_device:
    ...
    def draw_nmos(self):
        ...
        if self.gate_connection_up == "gate_connection_up_":
            gate_connection_text_up = self.gate_connection_up + str(self.conn_num)
        else:
            gate_connection_text_up = self.gate_connection_up

        if self.gate_connection_down == "gate_connection_down_":
            gate_connection_text_down = self.gate_connection_down + str(self.conn_num)
        else:
            gate_connection_text_down = self.gate_connection_down

        if self.gate_connection == "gate_connection_":
            gate_connection_text_down = self.gate_connection + str(self.conn_num)
        ...
        if (
            ...
            and self.connection != 2
            and self.connection != 3
            and self.connection != 4
        ):
        ...
        nsdm_box = pya.Box(
            -npsdm_enc_diff,
            -npsdm_enc_diff,
            diffusion_total_width + npsdm_enc_diff,
        ...
        for finger in range(self.nf):
            gate_box = pya.Box(
                diffusion_width + x,
                -poly_diff_min_enc,
                channel_length + diffusion_width + x,
        ...
        free_spc_licon_gate = (
            contact_gate_width - (2 * num_horizontal_licon_gate - 1) * licon_size
        )
        if channel_length > contact_gate_width:
            ...
            free_spc_licon_gate = (
                channel_length - (2 * num_horizontal_licon_gate - 1) * licon_size
            )
        ...
        npc_box_down = pya.Box(
            contact_gate_box_down.p1.x - 1.6 * npc_enc_gate,
            contact_gate_box_down.p1.y - npc_enc_gate,
            contact_gate_box_down.p2.x + 1.6 * npc_enc_gate,
            contact_gate_box_down.p2.y + npc_enc_gate,
        )
        li_box_down = pya.Box(
            diff_box.p1.x,
        ...
        gate_connection_text_down = pya.Text(
            gate_connection_text_up, li_box_down.center().x, li_box_down.center().y
        )

        num_hori_licon_diff, free_space_licon_diff_hor = self.number_spc_contacts(
            diffusion_width, liconpoly_spc, licon_size, licon_size
        )
        ...
        for hor_licon_diff in range(num_hori_licon_diff):
            licon_between_fingers = pya.Box(
                ...
                free_space_licon_diff_hor / 2 + licon_size + sh1,
        ...
        li_between_fingers_path = pya.Path(
            [
                pya.Point(diffusion_width / 2, diff_box.p1.y),
        ...
        met1_between_fingers_path = pya.Path(
            [
                pya.Point(diffusion_width / 2, diff_box.p1.y - mcon_m1_enc),
                pya.Point(diffusion_width / 2, diff_box.p2.y + mcon_m1_enc),
            ],
            diffusion_width,
        )

        contact_gate_box_up = pya.Box(
            gate_box.center().x - contact_gate_width / 2,
        ...
        npc_box_up = pya.Box(
            contact_gate_box_up.p1.x - 1.6 * npc_enc_gate,
            contact_gate_box_up.p1.y - npc_enc_gate,
            contact_gate_box_up.p2.x + 1.6 * npc_enc_gate,
            contact_gate_box_up.p2.y + npc_enc_gate,
        )
        ...
        licon_diff_contact_left = pya.Box(
            diff_box.p1.x + diff_licon_enc_horzintal,
            free_space_diff_licon / 2,
            diff_licon_enc_horzintal + licon_size,
            free_space_diff_licon / 2 + licon_size,
        )
        li_diff_left = pya.Path(
            [
                pya.Point(licon_diff_contact_left.center().x, diff_box.p1.y),
                pya.Point(licon_diff_contact_left.center().x, diff_box.p2.y),
            ],
            li_width,
        )
        met1_diff_left = pya.Path(
            [
                pya.Point(
                    licon_diff_contact_left.center().x, diff_box.p1.y - mcon_m1_enc
                ),
                pya.Point(
                    licon_diff_contact_left.center().x, diff_box.p2.y + mcon_m1_enc
                ),
            ],
        ...
        for drain in range(math.ceil((self.nf + 1) / 2)):
            ...
            x_offset_drain = x_offset_drain + 2 * (channel_length + diffusion_width)
        ...
        for source in range(((self.nf + 1) - math.ceil((self.nf + 1) / 2))):
            ...
            x_offset_source = x_offset_source + 2 * (channel_length + diffusion_width)

        mcon_diff_left = pya.Box(
            licon_diff_contact_left.p1.x,
            0,
            licon_diff_contact_left.p1.x + mcon_size,
            mcon_size,
        )

        licon_diff_contact_right = pya.Box(
            diff_box.p2.x - diff_licon_enc_horzintal - licon_size,
            free_space_diff_licon / 2,
        ...
        li_diff_right = pya.Path(
            [
                pya.Point(licon_diff_contact_right.center().x, diff_box.p1.y),
                pya.Point(licon_diff_contact_right.center().x, diff_box.p2.y),
            ],
            li_width,
        )
        mcon_diff_right = pya.Box(
            ...
            0,
            licon_diff_contact_right.p1.x + mcon_size,
            mcon_size,
        )
        met1_diff_right = pya.Path(
            [
                pya.Point(
                    licon_diff_contact_right.center().x, diff_box.p1.y - mcon_m1_enc
                ),
                pya.Point(
                    licon_diff_contact_right.center().x, diff_box.p2.y + mcon_m1_enc
                ),
            ],
            m1_width + 2 * mcon_m1_enc,
        )
        ...
        num_ver_mcon_gates, free_spc_mcon_gates_v = self.number_spc_contacts(
            li_box_down.height(), mcon_m1_enc, mcon_spc, mcon_size
        )
        num_hor_mcon_gates, free_spc_mcon_gates_h = self.number_spc_contacts(
            li_box_down.width(), mcon_m1_enc_adjacent, mcon_spc, mcon_size
        )
        ...
        if self.connection != 3:
            if self.connection == 1 or self.connection == 2:
                ...
                for mcon in range(num_hor_mcon_gates):
                    ...
                    if (
                        self.connected_gates == 0
                    ):  # the gate has contacts but no connection
        ...
        if self.connection != 3:
            ...
            iteration_distance_group = 2 * self.n * (diffusion_width + channel_length)
        ...
        if self.nf:
            licon_between_fingers_arr = pya.CellInstArray(
                licon_between_fingers_cell.cell_index(),
                pya.Trans(pya.Point(0, 0)),
                pya.Vector(0, 2 * licon_size),
                pya.Vector(diffusion_width + channel_length, 0),
                num_ver_licon,
                self.nf + 1,
            )
        ...
        if self.nf:
            li_between_fingers_arr = pya.CellInstArray(
                li_between_fingers_cell.cell_index(),
                pya.Trans(pya.Point(0, 0)),
                pya.Vector(diffusion_width + channel_length, 0),
                pya.Vector(0, 0),
                self.nf + 1,
        ...
        if self.nf:
            mcon_diff_bet_fing = pya.CellInstArray(
                mcon_between_fingers_cell.cell_index(),
                pya.Trans(pya.Point(0, 0)),
                pya.Vector(diffusion_width + channel_length, 0),
                pya.Vector(0, mcon_spc + mcon_size),
                self.nf + 1,
                num_ver_mcon,
            )
        ...
        guard_ring_lower_left = pya.Point(
            diff_box.p1.x - 2 * npsdm_enc_diff - npsdm_spc_opposite - tap_width / 2,
