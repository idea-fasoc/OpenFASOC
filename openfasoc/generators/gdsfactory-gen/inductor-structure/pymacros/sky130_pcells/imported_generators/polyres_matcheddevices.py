    ...
    def draw_one_finger(self, width, length, lfx, lfy, precision, m1=1):
        ...
        if m1:
            self.draw_mcon_in_li(
                lfx, lfy, width * precision, poly_res_lfy - lfy, precision
            )
            self.draw_mcon_in_li(
        ...
        for i in range(1, self.ry + 1):
            if i % 2 == 1:
                returned_lfx, returned_lfy = self.draw_one_raw(
                    lfx, current_lfy, self.w, self.l, precision, self.rx, False
                )
            else:
                returned_lfx, returned_lfy = self.draw_one_raw(
                    lfx, current_lfy, self.w, self.l, precision, self.rx, True
                )
