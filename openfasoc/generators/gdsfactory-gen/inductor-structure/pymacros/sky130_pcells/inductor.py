...
class IndGenerator(pya.PCellDeclarationHelper):
    ...
    def triangular_shielding(self, W, S, Lhor, diffusion):
        ...
        xcor = xcor + (W + S)
        self.cell.shapes(met1).insert(
            pya.Box(xcor, ycor, -xcor + Lhor, ycor + Lver / 2 - S / 2)
        )
        self.cell.shapes(met1).insert(
            pya.Box(xcor, ycor + Lver, -xcor + Lhor, ycor + Lver / 2 + S / 2)
        )
        if Shielding_with_diffusion == 1:
            self.cell.shapes(nwell).insert(
                pya.Box(xcor, ycor, -xcor + Lhor, ycor + Lver / 2 - S / 2)
            ...
            N_Licon_hor = int(
                (-2 * xcor_check - 2 * Li_Encloses_Licon_Two_Sides + Licon_Spacing)
                / (Licon_Width_Length + Licon_Spacing)
            )
            Remaining_Licon_hor = (
                -2 * xcor_check
            ...
            N_Mcon_hor = int(
                (-2 * xcor_check - 2 * Met1_Encloses_Mcon_Two_Sides + Mcon_Spacing)
                / (Mcon_Width_Length + Mcon_Spacing)
            )
            Remaining_Mcon_hor = (
                -2 * xcor_check
    ...
    def produce_impl(self):
        ...
        self.cell.shapes(met4).insert(
            pya.Box(
                xcor - W / 2, ycor, xcor + Louter - (N - 1) * W - (N - 1) * S, ycor + W
            )
        )
