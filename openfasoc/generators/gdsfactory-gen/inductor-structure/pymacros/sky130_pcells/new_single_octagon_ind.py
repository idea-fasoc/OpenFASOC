...
class new_single_octagon_Generator(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
        ...
        self.param(
            "Lvert_shielding",
            self.TypeDouble,
            "vertical dimension for shielding",
            default=40,
        )
        self.param(
            "Lhor_shielding",
            self.TypeDouble,
            "horizontal dimension for shielding",
            default=40,
        )
        self.param(
            "diffusion_shielding", self.TypeBoolean, "Diffusion shielding", default=1
        )
            ...
            if i == N - 1:
                ...
                if N == 1:
                    self.cell.shapes(met5).insert(
                        pya.Box(-S, ycor - W / 2, -(S + W), Dist_input)
                    )
                else:
                    self.cell.shapes(met4).insert(
                        pya.Box(-S, ycor + W / 2, -(S + W), Dist_input / 2)
                    )
                    self.cell.shapes(via4).insert(
                        pya.Box(-S, ycor + W / 2, -(S + W), ycor - W / 2)
                    )
                    self.cell.shapes(via4).insert(
                        pya.Box(-S, Dist_input, -(S + W), Dist_input / 2)
                    )
