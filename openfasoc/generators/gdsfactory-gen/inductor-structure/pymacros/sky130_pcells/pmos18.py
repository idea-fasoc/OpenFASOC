    ...
    def produce_impl(self):
        ...
        if nf == 1:
            diffusion_total_width = (
                channel_length + diffusion_width_big + diffusion_width_small
            )
            # todo :correct the diffusion_total_width of singel finger

        diffusion_total_width = (
            nf * channel_length
            + (nf - 1) * diffusion_width_big
            + 2 * diffusion_width_big
        )
        ...
        free_spc_mcon_diff = channel_width - (
            num_ver_mcon * (mcon_spc + mcon_size) - mcon_spc
        )
        ...
        l_nsdm = self.layout.layer(
            nsdm_lay_num, nsdm_lay_dt
        )  # nsdm source drain impalnt
        l_psdm = self.layout.layer(
            psdm_lay_num, psdm_lay_dt
        )  # psdm source drain impaln
        l_licon = self.layout.layer(
        ...
        for finger in range(nf):
            gate_box = pya.Box(
                diffusion_width_big + x,
                -poly_diff_min_enc,
                channel_length + diffusion_width_big + x,
        ...
        gate_box = pya.Box(
            diffusion_width_big,
            -poly_diff_min_enc,
            channel_length + diffusion_width_big,
            channel_width + poly_diff_min_enc,
        )
        ...
        npc_box_down = pya.Box(
            contact_gate_box_down.p1.x - npc_enc_gate,
            contact_gate_box_down.p1.y - npc_enc_gate,
            contact_gate_box_down.p2.x + npc_enc_gate,
        ...
        num_hori_licon_diff, free_space_licon_diff_hor = self.number_spc_contacts(
            diffusion_width_big, liconpoly_spc, licon_size, licon_size
        )
        ...
        num_hori_mcon_diff, free_space_mcon_diff_hor = self.number_spc_contacts(
            diffusion_width_big, 0, mcon_spc, mcon_size
        )
        sh2 = 0
        ...
        li_between_fingers_path = pya.Path(
            [
                pya.Point(diffusion_width_big / 2, diff_box.p1.y),
        ...
        met1_between_fingers_path = pya.Path(
            [
                pya.Point(diffusion_width_big / 2, diff_box.p1.y - mcon_m1_enc),
        ...
        num_hor_mcon_gates, free_spc_mcon_gates_h = self.number_spc_contacts(
            li_box_down.width(), mcon_m1_enc, mcon_spc, mcon_size
        )
        shift_distance = (self.n) * (
            diffusion_width_big + channel_length
        ...
        multiple_transistors = pya.CellInstArray(
        ...
        if self.gr:
            ...
            num_licon_guard_vert, free_spc_licon_guard_left = self.number_spc_contacts(
                distance_cont_guard_vert, mcon_m1_enc, licon_size, licon_size
            )
            ...
            num_mcon_guard_vert, free_spc_mcon_guard_left = self.number_spc_contacts(
                distance_cont_guard_vert, mcon_m1_enc, mcon_spc, mcon_size
            )
            ...
            num_via_guard_vert, free_spc_via_guard_left = self.number_spc_contacts(
                distance_cont_guard_vert, via_met_enc, via_spc, via_size
            )
            ...
            for licon in range(num_licon_guard_hori):
                ...
                (
                    num_mcon_guard_hori,
                    free_spc_mcon_guard_hori,
                ) = self.number_spc_contacts(
                ...
                self.cell.shapes(l_nwell).insert(
                    psdm_guard_ring_path.bbox().enlarge(
                        nwell_extension, nwell_extension
                    )
                )
