...
class Via_newGenerator(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
        ...
        starting_layer = self.param(
            "starting_metal", self.TypeString, "choose the starting metal", default=-4
        )
        ...
        ending_layer = self.param(
            "ending_metal", self.TypeString, "choose the ending material", default=-4
        )
        ...
        self.param(
            "hv",
            self.TypeBoolean,
            "High Voltage works in case of ptap and ntap",
            default=False,
        )
    ...
    def draw_metals(self, width, length, starting_metal, ending_metal):
        ...
        for metal in range(starting_metal, ending_metal + 1):
            if metal == -4:
                self.cell.shapes(l_poly).insert(
                    pya.Path(
                        [pya.Point(0, 0), pya.Point(length, 0)],
                        width + 2 * poly_extension,
                    )
                )
                self.cell.shapes(l_npc).insert(
                    pya.Path(
                        [pya.Point(-npc_enc, 0), pya.Point(length + npc_enc, 0)],
                        width + 2 * npc_enc,
                    )
                )
                Pass = 1
            if metal == -3 and Pass != 1:
                self.cell.shapes(l_tap).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
                self.cell.shapes(l_psdm).insert(
                    pya.Path(
                        [
                            pya.Point(-npsdm_enc_tap, 0),
                            pya.Point(length + npsdm_enc_tap, 0),
                        ],
                        width + 2 * npsdm_enc_tap,
                    )
                )
                if self.hv:
                    self.cell.shapes(l_hvi).insert(
                        pya.Path(
                            [
                                pya.Point(-hv_enc_tap, 0),
                                pya.Point(length + hv_enc_tap, 0),
                            ],
                            width + 2 * hv_enc_tap,
                        )
                    )
            ...
            if metal == -2 and Pass != 1:
                ...
                self.cell.shapes(l_nsdm).insert(
                    pya.Path(
                        [
                            pya.Point(-npsdm_enc_tap, 0),
                            pya.Point(length + npsdm_enc_tap, 0),
            ...
            if metal == -1:
                self.cell.shapes(l_li).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )

            if metal == 0:
                self.cell.shapes(l_met1).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
            if metal == 1:
                self.cell.shapes(l_met2).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
            if metal == 2:
                self.cell.shapes(l_met3).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
            if metal == 3:
                self.cell.shapes(l_met4).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
            if metal == 4:
                self.cell.shapes(l_met5).insert(
                    pya.Path([pya.Point(0, 0), pya.Point(length, 0)], width)
                )
    ...
    def draw_vias(self, width, length, starting_metal, ending_metal):
        ...
        for i in range(starting_metal, ending_metal):
            if i == -2:
                ...
                num_licon_1, licon_free_spc_1 = self.number_spc_contacts(
                    width, met_licon_enc_1, licon_spc, licon_size
                )
                num_licon_2, licon_free_spc_2 = self.number_spc_contacts(
                    length, met_licon_enc_2, licon_spc, licon_size
                )
                licon_arr = pya.CellInstArray(
                    licon_cell.cell_index(),
                    pya.Trans(
                        pya.Point(
                            licon_free_spc_2 / 2, -width / 2 + licon_free_spc_1 / 2
                        )
                    ),
                    pya.Vector(licon_spc + licon_size, 0),
                    pya.Vector(0, licon_spc + licon_size),
                    num_licon_2,
                    num_licon_1,
                )
            ...
            if i == -1:
                ...
                num_mcon_1, mcon_free_spc_1 = self.number_spc_contacts(
                    width, met_mcon_enc_1, mcon_spc, mcon_size
                )
                num_mcon_2, mcon_free_spc_2 = self.number_spc_contacts(
                    length, met_mcon_enc_2, mcon_spc, mcon_size
                )
                mcon_arr = pya.CellInstArray(
                    mcon_cell.cell_index(),
                    pya.Trans(
                        pya.Point(mcon_free_spc_2 / 2, -width / 2 + mcon_free_spc_1 / 2)
                    ),
                    pya.Vector(mcon_spc + mcon_size, 0),
    ...
    def produce_impl(self):
        ...
        # create the shape
        self.draw_metals(
            self.width, self.length, self.starting_metal, self.ending_metal
        )
