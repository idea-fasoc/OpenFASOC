...
def create_Carray() -> Component:
    ...
    for layer in layers:
        Cstructure = gf.import_gds(gds_outdir + "/" +
            str(layer) + "_line_res.gds", flatten=True
        )
    ...
    for layer in layers:
        Cstructure = gf.import_gds(gds_outdir + "/" +
            str(layer) + "_via_chain.gds", flatten=True
        )
    ...
    for layer in layers:
        Cstructure = gf.import_gds(gds_outdir + "/" +
            str(layer) + "_thick_line_res.gds", flatten=True,
        )
