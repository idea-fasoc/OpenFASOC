...
def pmos_lvt_cmirror_netlist(
        ...
        multipliers: int
         ...
         fet_1_ref=netlist.connect_netlist(fet_1.info['netlist'], [('D', 'Iin'), ('S', 'VCC'), ('B', 'VCC')]) 
         fet_2_ref=netlist.connect_netlist(fet_2.info['netlist'], [('D', 'Iout'), ('S', 'VCC'), ('B', 'VCC')]) 
...
def pmos_lvt_cmirror(     
    ...
    drain_B1_via.move(fet_B_1_ref.ports["multiplier_0_drain_W"].center).movex(-0.28)
    ...
    if with_substrate_tap:
      ...
      substrate_tap_encloses =(2.5 * (substrate_tap_separation + top_level.xmax), 3 * (substrate_tap_separation + top_level.ymax))
...
cmirr = pmos_lvt_cmirror(sky130_mapped_pdk)
cmirr.show()
"""
magic_drc_result = sky130_mapped_pdk.drc_magic(cmirr, cmirr.name)
if magic_drc_result:
