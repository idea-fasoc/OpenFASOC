#!/usr/bin/env python3
"""
8-Hour Runtime-Aware FVF Dataset Generator
Incorporates the proven approach from final_robust_sweeper.py to handle DRC/LVS issues.
Uses the 8-hour runtime-aware parameter set (10,886 samples) generated from budgets_8h_runtime_aware_measuredTp_dpCorrected.json.
Part of the complete 40,814 sample budget allocation across all PCells.
"""
...
def load_fvf_parameters(num_samples=None):
    ...
    parameter_file = "/home/arnavshukla/OpenFASOC/openfasoc/generators/glayout/glayout/flow/blocks/elementary/LHS/gen_params_8h_runtime_aware/fvf_params.json"
...
def main():
    """Main function for 10,886-sample FVF dataset generation using 8-hour runtime-aware LHS methodology"""
    print("🚀 FVF Dataset Generation - 10,886 LHS Samples (8-hour runtime-aware)")
    print("=" * 80)
    ...
    print(f"   Budget allocation: Part of 8-hour runtime-aware plan")
    print(f"   Assuming 10.748 seconds per sample average (from budget)")
    print(f"\n📁 Output will be saved to: fvf_dataset_8h_runtime_aware")
    print(f"💾 Checkpoints will be saved every 100 samples")
    print(f"🔄 Can resume from checkpoint if interrupted")
    
    # Get user confirmation
    print(f"\n⚠️  This is a long running process (part of 8-hour budget plan)!")
    print(f"   Recommended: Run in a screen/tmux session")
    print(f"   Using 8-hour runtime-aware LHS sampling for optimal parameter space coverage")
    print(f"   This is the FVF portion of the full 40,814 sample budget")
    ...
    success, passed, total = run_dataset_generation(
        actual_samples, 
        "fvf_dataset_8h_runtime_aware",
        checkpoint_interval=100,  # Save every 100 samples for large dataset
        resume_from_checkpoint=True
    )
    
    if success:
        print(f"\n🎉 8-hour runtime-aware FVF dataset generation completed successfully!")
        print(f"📊 Final results: {passed:,}/{total:,} samples successful")
        print(f"📁 Dataset saved to: fvf_dataset_8h_runtime_aware/")
        return True
    else:
        print(f"\n⚠️ Dataset generation completed with issues")
        print(f"📊 Final results: {passed:,}/{total:,} samples successful")
        print(f"💡 Check logs and results in: fvf_dataset_8h_runtime_aware/")
        return False

if __name__ == "__main__":
    success = main()
    if success:
        print("\n✅ 8-hour runtime-aware FVF dataset generation pipeline completed!")
        print("🎯 10,886 samples with optimal parameter space coverage generated!")
        print("📊 Part of the complete 40,814 sample 8-hour budget plan")
    else:
