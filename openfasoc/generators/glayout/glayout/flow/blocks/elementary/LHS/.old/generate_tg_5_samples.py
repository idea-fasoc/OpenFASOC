#!/usr/bin/env python3
"""
Transmission Gate Dataset Generator - 5 Samples Version
Based on the proven approach from generate_fvf_360_robust_fixed.py.
Generates 5 synthetic parameter combinations for transmission gate and monitors runtime.
"""
...
def generate_tg_parameters(num_samples=5):
    """Generate synthetic transmission gate parameters for testing"""
    
    # Define parameter ranges for transmission gate
    # Based on typical MOSFET sizing for digital applications
    parameters = []
    
    # Sample 1: Basic small transmission gate
    parameters.append({
        "width": (1.0, 2.0),        # NMOS=1um, PMOS=2um (2:1 ratio for balanced)
        "length": (0.15, 0.15),     # Minimum length for both
        "fingers": (1, 1),          # Single finger each
        "multipliers": (1, 1),      # Single device each
        "substrate_tap": False
    })
    
    # Sample 2: Larger transmission gate with multiple fingers
    parameters.append({
        "width": (2.0, 4.0),        # Larger devices
        "length": (0.18, 0.18),     # Slightly longer channel
        "fingers": (2, 2),          # Multi-finger for better matching
        "multipliers": (1, 1),
        "substrate_tap": True       # Include substrate tap
    })
    
    # Sample 3: High drive strength transmission gate
    parameters.append({
        "width": (3.0, 6.0),        # High W/L ratio
        "length": (0.15, 0.15),     # Minimum length for speed
        "fingers": (3, 3),          # More fingers
        "multipliers": (2, 2),      # Multiple devices in parallel
        "substrate_tap": False
    })
    
    # Sample 4: Conservative longer channel design
    parameters.append({
        "width": (1.5, 3.0),        # Medium sizing
        "length": (0.5, 0.5),       # Longer channel for better matching
        "fingers": (1, 1),
        "multipliers": (1, 1),
        "substrate_tap": True
    })
    
    # Sample 5: Complex multi-finger, multi-device design
    parameters.append({
        "width": (4.0, 8.0),        # Large devices
        "length": (0.28, 0.28),     # Intermediate length
        "fingers": (4, 4),          # Many fingers
        "multipliers": (2, 2),      # Multiple parallel devices
        "substrate_tap": True       # With substrate tap
    })
    
    logger.info(f"Generated {len(parameters)} synthetic transmission gate parameter combinations")
    ...
    for i, params in enumerate(parameters, 1):
        ...
        tap = "Yes" if params["substrate_tap"] else "No"
        
        logger.info(f"  Sample {i}: NMOS({nmos_w:.1f}μm/{nmos_l:.2f}μm, {nmos_f}f×{nmos_m}), "
                   f"PMOS({pmos_w:.1f}μm/{pmos_l:.2f}μm, {pmos_f}f×{pmos_m}), Sub-tap: {tap}")
...
def run_dataset_generation(n_samples, output_dir):
    """Run the dataset generation for n_samples"""
    ...
    # Generate parameters
    parameters = generate_tg_parameters(n_samples)
    ...
    for i in range(n_samples):
        ...
        # Progress update for each sample (since we only have 5)
        success_rate = sum(1 for r in results if r["success"]) / len(results) * 100
        elapsed = time.time() - total_start
        
        logger.info(f"📈 Progress: {i+1}/{n_samples} ({(i+1)/n_samples*100:.0f}%) - "
    ...
    # Log individual sample performance
    logger.info(f"\n📊 Individual Sample Performance:")
    for r in results:
        status = "✅ PASS" if r["success"] else "❌ FAIL"
        time_str = f"{r['execution_time']:.1f}s"
        if r["success"]:
            area = r.get("area_um2", 0)
            logger.info(f"   Sample {r['sample_id']:02d}: {status} - {time_str} - Area: {area:.2f}μm²")
        else:
            error = r.get("error", "Unknown error")
            logger.info(f"   Sample {r['sample_id']:02d}: {status} - {time_str} - Error: {error}")
    ...
    return success_rate >= 60, len(successful), len(results)  # Lower threshold for small sample

def main():
    """Main function for 5-sample Transmission Gate dataset generation"""
    print("🚀 Transmission Gate Dataset Generation - 5 Synthetic Samples")
    print("=" * 65)
    
    n_samples = 5
    output_dir = "tg_dataset_5_samples"
    
    print(f"📊 Generating {n_samples} synthetic transmission gate parameter combinations")
    ...
    for i, params in enumerate(parameters, 1):
        ...
        nmos_f, pmos_f = params["fingers"]
        nmos_m, pmos_m = params["multipliers"]
        tap = "✓" if params["substrate_tap"] else "✗"
        print(f"   {i}. NMOS: {nmos_w:.1f}μm/{nmos_l:.2f}μm×{nmos_f}f×{nmos_m} | "
              f"PMOS: {pmos_w:.1f}μm/{pmos_l:.2f}μm×{pmos_f}f×{pmos_m} | Sub-tap: {tap}")
    
    print(f"\n🤔 Continue with transmission gate dataset generation? (y/n): ", end="")
    ...
    success, passed, total = run_dataset_generation(n_samples, output_dir)
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("🎯 5 samples with varied parameter combinations generated!")
    else:
