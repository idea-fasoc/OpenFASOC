...
if __name__ == "__main__":
   ...
   output_dir = os.path.join(os.path.dirname(__file__), "opamp_180_params")
   os.makedirs(output_dir, exist_ok=True)
  
   for pcell, samples in all_samples.items():
       # Match naming style used for current-mirror dataset (…_params.json)
       fname = (
           "opamp_params.json" if pcell == "opamp" else f"{pcell}_parameters.json"
       )
