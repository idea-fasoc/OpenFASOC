#!/usr/bin/env python3
"""
Current Mirror Dataset Generation Script - 3164 LHS Samples
Based on generate_current_mirror_360_robust_fixed.py but modified for automatic generation
using parameters from fvf_2000_lhs_params/curr_mirror_params.json
"""
...
# Environment setup similar to the working script
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('current_mirror_3164_dataset.log')
    ]
)
logger = logging.getLogger(__name__)

# Global PDK instance to reuse across trials
...
def load_current_mirror_parameters_3164():
    """Load current mirror parameters from the 3164 sample LHS file"""
    parameter_file = "fvf_2000_lhs_params/curr_mirror_params.json"
    ...
    logger.info(f"Loaded {len(parameters)} parameter combinations from {parameter_file}")
    return parameters
...
def run_single_evaluation(trial_num, params, output_dir):
    ...
    try:
        ...
        # Use a stable PDK instance across all trials
        pdk = get_global_pdk()
...
def run_dataset_generation(n_samples: int, output_dir: str, *, checkpoint_interval: int = 100, resume_from_checkpoint: bool = True):
    ...
    parameters = load_current_mirror_parameters_3164()
...
def main():
    """Main entry-point – automatically generate 3164 sample current mirror dataset"""
    print("🚀 Current Mirror Dataset Generation - 3164 LHS Samples")
    print("=" * 70)

    # Set parameters for 3164 samples
    n_samples = 3164
    output_dir = "cm_dataset_3164_lhs"
    
    print(f"📊 Generating {n_samples:,} current mirror samples")
    print(f"📁 Output directory: {output_dir}")
    print(f"📄 Using parameters from: fvf_2000_lhs_params/curr_mirror_params.json")
    
    estimated_time_hours = (n_samples * 15) / 3600  # ~15s/sample conservative
    print(f"\n⏱️  Estimated completion time: {estimated_time_hours:.1f} hours")
    print("   Assuming 15 seconds per sample average")
    print("💾 Checkpoints will be saved every 100 samples")
...
if __name__ == "__main__":
    success = main()
    if success:
        print("\n✅ Current Mirror 3164 dataset generation pipeline completed!")
    else:
