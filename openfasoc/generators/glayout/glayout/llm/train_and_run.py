...
def load_model_and_tokenizer(device: str, lora: bool = True) -> tuple:
    ...
    if not qlora:
        ...
        model = AutoModelForCausalLM.from_pretrained(modelname, token=accesstoken, quantization_config=BitsAndBytesConfig(load_in_8bit=True))
...
def run_llm_normal(
    ...
    """
    model.eval()
    inputs = tokenizer.apply_chat_template([{"role": "user", "content": prompt}], tokenize=True, add_generation_prompt=True)
    inputs.to(device)
