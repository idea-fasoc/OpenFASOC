    ...
    if args.mode != "post":
        if args.simtype == "postPEX":
            ...
            for i, figure in enumerate(figures):
                ...
                fig_dc_results(postPEX_sim_dir + "/isweep.raw").savefig(
                    args.outputDir + "/dc.png"
                )
                max_load = user_specs["imax"]
                load = max_load * 1000
                fig_load_change_results(
                    postPEX_sim_dir + "/" + str(load) + "mA_output_load_change.raw",
                    load,
                ).savefig(args.outputDir + "/load_change.png")
        if args.simtype == "prePEX":
            for s in range(len(sim)):
            ...
            for i, figure in enumerate(figures):
                figure.savefig(current_freq_results + "/" + figure_names[i] + ".png")
                fig_dc_results(prePEX_sim_dir + "/isweep.raw").savefig(
                    args.outputDir + "/dc.png"
                )
                max_load = user_specs["imax"]
                load = max_load * 1000
                fig_load_change_results(
                    prePEX_sim_dir + "/" + str(load) + "mA_output_load_change.raw", load
                ).savefig(args.outputDir + "/load_change.png")
