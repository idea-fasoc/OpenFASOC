...
add_global_connection -defer_connection -net VDD -inst_pattern {temp_analog_1.*} -pin_pattern {VPWR|VPB} -power
add_global_connection -defer_connection -net VIN -inst_pattern {temp_analog_0.*} -pin_pattern {VPWR|VPB} -power
add_global_connection -defer_connection -net VSS -inst_pattern {.*} -pin_pattern {VGND|VNB} -ground

# Manually add connections for tap and encap cells
foreach inst $caps_analog {
  add_global_connection -defer_connection -net VIN -inst_pattern [$inst getName] -pin_pattern {VPWR|VPB} -power
}
foreach inst $caps_core {
  add_global_connection -defer_connection -net VDD -inst_pattern [$inst getName] -pin_pattern {VPWR|VPB} -power
}
...
add_pdn_stripe -grid stdcell -layer met5 -width 1.6 -pitch 15.0 -extend_to_core_ring -starts_with GROUND -number_of_straps 4
