...
parser = argparse.ArgumentParser(description="Generates or updates rules file for CI.")
...
parser.add_argument(
    "-f", "--failing", action="store_true", default=False, help="Update failing rules."
)
...
for field, option in rules_dict.items():
    if field not in metrics.keys():
        print(f"[ERROR] Metric {field} not found in " f"metrics file: {metrics_file}.")
        sys.exit(1)
    ...
    if rule_value == 0 and (
        ...
        rule_value = ceil(metrics["placeopt__design__instance__count__stdcell"] * 0.1)
    ...
    if OLD_RULES is not None and field in OLD_RULES.keys():
        ...
        if args.failing and not compare(metrics[field], old_rule["value"]):
            UPDATE = True
            print(
                f"[INFO] Updating failing rule {field} "
                f"from {old_rule['value']} to {rule_value}."
            )
