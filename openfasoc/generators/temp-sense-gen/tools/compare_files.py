def compare_files(template_file, result_file) -> int:
    ...
    Args:
    - 'template_file' (string): The stored template file containing temperature sensor simulation results (frequency, power, error)
    - 'result_file' (string): The result file generated by the currently run simulations
    ...
    with open(template_file, 'r') as template, open(result_file, 'r') as result:
        ...
        for template_line, result_line in zip(template, result):
            ...
            if freq_diff > errors['frequency']['max'] or power_diff > errors['power']['max'] or error_diff > errors['error']['max']:
                print("percent deviation in frequency = " + str(freq_diff) + "%, max allowable deviation is ", str(errors['frequency']['max']) + "%.")
                print("percent deviation in power = " + str(power_diff) + "%, max allowable deviation is ", str(errors['power']['max']) + "%.")
                print("percent deviation in error = " + str(error_diff) + "%, max allowable deviation is ", str(errors['error']['max']) + "%.")
                print("Error occurred at temperature " + str(template_data[0]) + " degrees.")
                return 2
