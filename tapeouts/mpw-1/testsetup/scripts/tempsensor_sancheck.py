...
while True:
    sel_ctr = input(
        "Please set the length of measurement time window: enter n = 0 ~ 15, 16*(2^n) CLK_REF cycles.\n"
    )
...
while True:
    sel_design = input(
        "Please select between different NMOS header designs: enter 0 (gate connected to ground) or 1 (gate connected to source).\n"
    )
...
while True:
    sel_inst = input(
        "Please select between different RingOsc inverter-chain length: enter 0 ~ 3.\n"
    )
